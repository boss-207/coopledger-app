...
// ── Données de démo si Firebase vide ──


const HISTORIQUE_VOTES = [
  { id: 'h1', titre: "Achat semences maïs", montant: 180000, resultat: 'approuve', pourcentage: 81, date: new Date(Date.now() - 604800000) },
  { id: 'h2', titre: "Réparation tracteur", montant: 320000, resultat: 'rejete', pourcentage: 38, date: new Date(Date.now() - 1209600000) },
  { id: 'h3', titre: "Formation membres", montant: 95000, resultat: 'approuve', pourcentage: 76, date: new Date(Date.now() - 1814400000) },
];
                  ...
                  <div key={h.id} className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-400 mt-0.5">
                        {h.pourcentage}% {h.resultat === 'approuve' ? 'OUI' : 'NON'} · {formatDate(h.date)}
                      </p>
                    </div>
                    <p className={`text-xs font-bold flex-shrink-0 ${
                      h.resultat === 'approuve' ? 'text-green-600' : 'text-red-500'
                    }`}>
